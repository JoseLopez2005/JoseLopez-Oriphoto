...
**********************************************/
	...
	async function allProductsList(){
		...
		listProduct.setAttribute('class', 'boutique__results');
		//Ajout de la section dans le HTML
		let boutique = document.getElementById('boutique');
		boutique.appendChild(listProduct);
		...
		{ 
      	...
      	produitBlock.setAttribute('class', 'produit-item');
      	produitImage.setAttribute('src', produit.imageUrl);
      	produitImage.setAttribute('alt', 'image de l\'appareil');
      	produitImage.setAttribute('class', 'produit-item__img');
      	produitLink.setAttribute('href', 'product.html?id=' + produit._id);
      	produitLink.setAttribute('class', 'produit-item__bouton');
        ...
        produitBlock.appendChild(produitLink);
      	...
      	produitPrix.textContent = 'Prix : ' + numberWithSpace(produit.price / 100) + ' €';
...
async function detailProduit(){
    ...
    document.getElementById('priceProduct').innerHTML = 'Prix : ' + numberWithSpace(produitSelected.price / 100) + '€';
    ...
    switch(cameras){
    	...
    	default:
    	alert('Administration : Veuillez bien renseigner la variable cameras ligne 2 du fichier script.js');
    }
};
...
addition = () =>{
    ...
    if(JSON.parse(localStorage.getItem('userPanier')).length > 0){
      ...
      facture.setAttribute('id', 'liste-panier')
      ...
      let colonneMontant = document.createElement('th')
      ...
      colonneNom.textContent = 'Produit';
      ...
      ligneTableau.appendChild(colonneMontant);
      colonneMontant.textContent = 'Montant';
      //ligneTableau.appendChild(colonneRemove);
      //colonneRemove.textContent = 'Annuler un produit';
      

      //Pour chaque produit du panier, on créé une ligne avec le nom, le prix, la quantité, ...
      ...
      JSON.parse(localStorage.getItem('userPanier')).forEach((produit)=>{
        ...
        let montantProduit = document.createElement('td');
        ...
        ligneProduit.appendChild(montantProduit);
        ligneProduit.appendChild(removeProduit);

        //Contenu des lignes
        nomProduit.innerHTML = produit.name;
        prixUnitProduit.textContent = numberWithSpace(produit.price / 100) + ' €';
        quantiteProduit.textContent = 1

        /*if(facture.hasOwnProperty(cameras._id)) {
          quantiteProduit = facture[cameras._id].cantidad +1
        }
        facture[cameras._id] = {...produit}*/
    });
